      ...
      subs
        ...
        .map(async (sub) => {
          ...
          try {
            ...
            console.error(
              `Error converting currency for subscription ${sub._id}:`,
              error
            );
            return {
              ...sub.toObject(),
...
SubscriptionHandler.post("/addSub", async (req, res) => {
  try {
    ...
    const categoriesToProcess = Array.isArray(body.category)
      ? body.category
      : ["Uncategorized"];
    const uniqueCategories = [
      ...new Set(categoriesToProcess.filter((cat) => cat && cat.trim())),
    ];
